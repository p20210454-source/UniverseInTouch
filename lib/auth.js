...
function assertSecureConfig() {
  ...
  if (!pwd || !String(pwd).trim()) {
    console.error('FATAL: Set ADMIN_PASSWORD in .env (see .env.example).');
    process.exit(1);
  }
  if (pwd === 'changeme') {
...
function validateAdminCredentials(username, password) {
  ...
  return safeEqual(user, ADMIN_USERNAME) && safeEqual(String(password || ''), process.env.ADMIN_PASSWORD);
}
